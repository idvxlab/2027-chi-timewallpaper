...
3. Calls Gemini with base image + mask + elder-region edit prompt driven by transcript.
...
def _call_gemini_inpaint(
    ...
    """
    Call Gemini with base image + mask for regional inpaint.
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            f"Gemini inpaint error {response.status_code}: {response.text}"
        )
...
async def edit_elder_region_with_transcript(
    *,
    ...
    try:
        ...
        print(f"[wallpaper.elder-edit.service] Gemini call failed: {exc}")
        return {"imageUrl": ""}
    ...
    final = _composite_region(base_image, gemini_result, elder_mask)
