...
def test_agent_run_audio_mvp_returns_four_agent_outputs() -> None:
    settings.provider_api_key = ""
    settings.provider_api_base_url = ""
    ...
    assert scaffold["inputBoundary"] == "只读取短期语义表、长期关系表和语义图谱；不回看原始文本，不重新做语言理解。"
    ...
    assert scaffold["composition"]["layoutContract"]["middlePathZoneRatio"] == 0.20
    assert scaffold["composition"]["layoutContract"]["lowerLeftZoneRatio"] == 0.30
    assert "1/2到2/3" in scaffold["composition"]["characterFramingContract"]["scaleRule"]
    assert scaffold["generationParameters"]["characterAreaOccupancyWithinEachZone"] == "1/2_to_2/3"
    ...
    assert result["semanticMapping"]["graphVersion"] == "chi-semantic-visual-graph-v3"
    ...
    assert image["generationMode"] == "mask_image2image_mvp"
    assert "relation_path" in image["changedRegions"]
    assert "left_bottom_current_actor" in image["changedRegions"]
    assert "upper_right_other_side" in image["changedRegions"]
    assert image["assetMetadata"]["parent_image"] == "/generated/day-1.png"
    assert "mask" in image["assetMetadata"]["prompt"] or image["assetMetadata"]["masks"]
    ...
    with SessionLocal() as session:
        ...
        assert relationship.parent_role == "mother"
        assert relationship.child_user_id == "child-demo"
        assert relationship.child_role == "daughter"
        state = (
...
def test_agent_run_text_mvp_skips_asr_and_generates_prompt() -> None:
    settings.provider_api_key = ""
    settings.provider_api_base_url = ""
